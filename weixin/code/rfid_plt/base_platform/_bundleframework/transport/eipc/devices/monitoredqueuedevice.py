...
from eipc.kernel import QUEUE, FORWARDER, EIPCError
from eipc.devices.basedevice import Device,ThreadDevice,ProcessDevice
from eipc.devices.monitoredqueue import monitored_queue
...
class MonitoredQueueBase(object):
    ...
    def bind_mon(self, addr):
        """Enqueue EIPC address for binding on mon_socket.
    ...
    def connect_mon(self, addr):
        """Enqueue EIPC address for connecting on mon_socket.
...
class MonitoredQueue(MonitoredQueueBase, Device):
    ...
    *Warning* as with most 'threadsafe' Python objects, this is only
    ...
    See eipc.devices.Device for most of the spec. This subclass adds a
    <method>_mon version of each <method>_{in|out} method, for configuring the
    monitor socket.

    A MonitoredQueue is a 3-socket EIPC Device that functions just like a
