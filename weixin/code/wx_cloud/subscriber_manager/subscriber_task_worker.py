...
class AutoWanIpCheckTimeoutHandler(bf.TimeOutHandler):
    ...
    def time_out(self):
        ...
        if url is not None and url.find(wan_ip) < 0:
            ...
            ret = self.get_worker().get_app().mod_itf_cfg_info(new_url, bizs[0].access_token)
...
class SubscriberTaskWorker(bf.Worker):
    ...
    def idle(self, total_ready_frames):
        while self._msg_queue.empty() is not True:
            msg = self._msg_queue.get(True, 0.1)
            
            if msg[0] is self.TASK_TYPE_PUSH_ARTICLE:
                ...
                back_pic = msg_params_def.PORTAL_IMG_FILE_LOCAL_PATH_PREFIX + msg_params_def.PORTAL_IMG_FILE_USER_SHARE_MSG
